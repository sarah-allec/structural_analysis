...
class GofR():
    ...
    def rho( self, cell, positions, ngrid, sigma ):
        ...
        curr_r_ij = self.r_ij(cell,positions) 
        r_grid = np.linspace(0.5,ceil(max(curr_r_ij)),ngrid)
        return [r_grid, np.divide( 2*np.sum( self.f(r_grid, curr_r_ij, sigma), axis = 0 ), N)]

    def little_g( self, step, sigma=0.15, ngrid = 100 ):
